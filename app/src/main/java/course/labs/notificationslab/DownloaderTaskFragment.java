...
public class DownloaderTaskFragment extends Fragment
{
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        // TODOx: Create new DownloaderTask that "downloads" data
        DownloaderTask task = new DownloaderTask();

        // TODOx: Retrieve arguments from DownloaderTaskFragment
    ...
    // TODOx: Implement an AsyncTask subclass called DownLoaderTask.
    // This class must use the downloadTweets method (currently commented
    // out). Ultimately, it must also pass newly available data back to
    // the hosting Activity using the DownloadFinishedListener interface.
    class DownloaderTask extends AsyncTask<ArrayList<Integer>, Integer, String[]>
    {
        @Override
        protected String[] doInBackground(ArrayList<Integer>... params)
        {
            ArrayList<Integer> friendResIds = params[0];

            Integer[] ret = new Integer[friendResIds.size()];
            for (int i = 0; i < ret.length; i++)
            {
                ret[i] = friendResIds.get(i).intValue();
            }

            String[] tweets = downloadTweets(ret);

            return tweets;
        }

        @Override
        protected void onPostExecute(String[] theTweets)
        {
            mCallback.notifyDataRefreshed(theTweets);
        }
    }

    // TODOx: Uncomment this helper method
    ...
    private void notify(final boolean success)
    {
        ...
        mContext.sendOrderedBroadcast(new Intent(
                ...
                new BroadcastReceiver()
                {
                    ...
                    {
                        // TODOx: Check whether or not the MainActivity received the broadcast
                        ...
                        if(resultCode != MainActivity.IS_ALIVE)
                        {
                            ...
                            // TODOx: Set the notification View's text to reflect whether the download completed successfully
